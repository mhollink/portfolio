import {type Client, clients, type Project} from '~components/sections/experiences/clients.ts';
        ...
        <div className="client-project">
            ...
            <Flex gap="xs" margin="0.5rem 0 0" wrap={true}>
                {project.technologies.map((t) => (
                    <Chip label={t} size="small" appearance="outlined"/>
                ))}
            </Flex>
        </div>
    )
    ...
    return (
        <div className="timeline__item">
            ...
            <div className="timeline__item__content">
                    <Typography variant="h3" className="timeline__item__title">{client.company}</Typography>
                ...
                <Typography className="timelinte__item__description">{t(client.description)}</Typography>

                <div className="timeline__item__projects">
                    {client.projects.map((p) => <Project project={p} key={p.title}/>)}
                </div>
            </div>
        </div>
...
export const Clients = () => {
    return (
        <div className="timeline">
            {clients.map((client) => <Client key={client.company} client={client}/>)}
        </div>
    );
};
